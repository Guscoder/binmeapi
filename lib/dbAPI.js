...
const getBinRequests = async (binId) => {
  const GET_BINS =
    "SELECT requests.payload, requests.date_received, requests.bins_id, requests.method, requests.headers FROM requests WHERE bins_id = $1";
...
const addRequest = async (binId, payload, method, headers, date) => {
  const INSERT_REQUEST =
    "INSERT INTO requests" +
    "(bins_id, payload, method, headers, date) " +
    "VALUES ($1, $2, $3, $4, $5)";
